<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <h2 class="text-xl font-semibold text-gray-800">Create Monitor</h2>
            <a href="{{ route('monitors.index') }}" class="text-sm text-indigo-600 hover:text-indigo-900">&larr; Back to Monitors</a>
        </div>
    </x-slot>

    <div class="max-w-2xl">
        <form method="POST" action="{{ route('monitors.store') }}" class="bg-white shadow rounded-lg border border-gray-200">
            @csrf
            ...
            <div class="px-6 py-5 space-y-5">
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        ...
                        <select id="type" name="type" class="mt-1 block w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm" required>
                            ...
                            <option value="https" {{ old('type', 'https') === 'https' ? 'selected' : '' }}>HTTPS</option>
                            <option value="tcp" {{ old('type') === 'tcp' ? 'selected' : '' }}>TCP</option>
                ...
                <div class="grid grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <select id="check_interval_seconds" name="check_interval_seconds" class="mt-1 block w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm" required>
                            ...
                            <option value="{{ $interval }}" {{ old('check_interval_seconds', 60) == $interval ? 'selected' : '' }}>{{ $interval }}s</option>
                            @endforeach
                        </select>
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        ...
                        <x-text-input id="expected_status_code" name="expected_status_code" type="number" class="mt-1 block w-full" :value="old('expected_status_code', '200')" />
                    ...
                    <div>
                        ...
                        <x-text-input id="expected_keyword" name="expected_keyword" type="text" class="mt-1 block w-full" :value="old('expected_keyword')" placeholder="Optional" />
            ...
            <div class="px-6 py-5 border-t border-gray-200 bg-gray-50">
                ...
                <div class="space-y-3">
                    <label class="flex items-center gap-3">
                        <input type="checkbox" name="ssl_enabled" value="1" {{ old('ssl_enabled') ? 'checked' : '' }} class="rounded border-gray-300 text-indigo-600 focus:ring-indigo-500">
                        <span class="text-sm text-gray-700">Enable SSL Certificate Monitoring</span>
                    </label>
                    <label class="flex items-center gap-3">
                        <input type="checkbox" name="domain_enabled" value="1" {{ old('domain_enabled') ? 'checked' : '' }} class="rounded border-gray-300 text-indigo-600 focus:ring-indigo-500">
            ...
            <div class="px-6 py-5 border-t border-gray-200">
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div class="space-y-3">
                        <label class="flex items-center gap-3">
                            <input type="checkbox" name="alert_on_down" value="1" {{ old('alert_on_down', true) ? 'checked' : '' }} class="rounded border-gray-300 text-indigo-600 focus:ring-indigo-500">
                            <span class="text-sm text-gray-700">Alert on Down</span>
                        </label>
                        <label class="flex items-center gap-3">
                            <input type="checkbox" name="alert_on_up" value="1" {{ old('alert_on_up', true) ? 'checked' : '' }} class="rounded border-gray-300 text-indigo-600 focus:ring-indigo-500">
                            <span class="text-sm text-gray-700">Alert on Recovery</span>
                        </label>
                        <label class="flex items-center gap-3">
                            <input type="checkbox" name="alert_on_ssl_expiry" value="1" {{ old('alert_on_ssl_expiry', true) ? 'checked' : '' }} class="rounded border-gray-300 text-indigo-600 focus:ring-indigo-500">
                            <span class="text-sm text-gray-700">Alert on SSL Expiry</span>
                        </label>
                        <label class="flex items-center gap-3">
                            <input type="checkbox" name="alert_on_domain_expiry" value="1" {{ old('alert_on_domain_expiry', true) ? 'checked' : '' }} class="rounded border-gray-300 text-indigo-600 focus:ring-indigo-500">
                    ...
                    <div class="space-y-4">
                        <div>
                            ...
                            <x-text-input id="ssl_alert_threshold_days" name="ssl_alert_threshold_days" type="number" class="mt-1 block w-full" :value="old('ssl_alert_threshold_days', 20)" />
                        ...
                        <div>
                            ...
                            <x-text-input id="domain_alert_threshold_days" name="domain_alert_threshold_days" type="number" class="mt-1 block w-full" :value="old('domain_alert_threshold_days', 20)" />
            ...
            <div class="px-6 py-4 border-t border-gray-200 flex justify-end gap-3">
                ...
                <x-primary-button>Create Monitor</x-primary-button>
            </div>
        </form>
    </div>
</x-app-layout>
